...
public sealed class DiacriticComposerDialog : Form
{
    ...
    //打开窗口时已经存在的Diacritics。
    private readonly List<string> initialDiacritics;

    //防止初始化勾选项目时触发多余的预览更新。
    private bool isLoadingDiacritics;

    public DiacriticComposerDialog(string baseSymbol, IEnumerable<string>? selectedDiacritics = null)
    {
        this.baseSymbol = IpaComposer.NormalizeSymbol(baseSymbol);
        initialDiacritics = selectedDiacritics?.ToList() ?? new List<string>();
    ...
    private void BuildLayout()
    {
        ...
        {
            if (isLoadingDiacritics) return;

            //ItemCheck发生时CheckedItems还没有更新，因此延迟到消息循环下一次执行。
    ...
    {
        isLoadingDiacritics = true;
        diacriticList.Items.Clear();

        foreach (IpaDiacritic diacritic in IpaDiacritics.All)
        {
            DiacriticListItem item = new(diacritic);
            bool isSelected = initialDiacritics.Any(existing =>
                IpaComposer.AreEquivalent(existing, diacritic.Symbol));

            diacriticList.Items.Add(item, isSelected);
        }

        isLoadingDiacritics = false;
    }

    private void UpdatePreview()
    {
        List<string> selected = GetSelectedDiacritics();
    ...
    private List<string> GetSelectedDiacritics()
    {
        ...
        foreach (object checkedItem in diacriticList.CheckedItems)
        {
